...
// func NewMailer(host, port, email, password string, useAuth bool) *Mailer {
// 	return &Mailer{
// 		SMTPHost:     host,
// 		SMTPPort:     port,
// 		AuthEmail:    email,
...
// 	subject := "Доступ к корпоративному порталу"
// 	body := fmt.Sprintf("Ваш логин: %s\nВаш пароль: %s", login, password)
// 	header := fmt.Sprintf("From: %s\nTo: %s\nSubject: %s\n\n", m.AuthEmail, to, subject)
// 	msg := []byte(header + body)

// 	var auth smtp.Auth
// 	if m.UseAuth {
// 		auth = smtp.PlainAuth("", m.AuthEmail, m.AuthPassword, m.SMTPHost)
// 	}

// 	err := smtp.SendMail(m.SMTPHost+":"+m.SMTPPort, auth, m.AuthEmail, []string{to}, msg)
// 	if err != nil {
// 		return fmt.Errorf("ошибка отправки письма: %v", err)
// 	}
// 	return nil
